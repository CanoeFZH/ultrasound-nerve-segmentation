...
def train_and_predict():
    ...
    y_train= np.array([ int(mask_not_blank(ele)) for ele in y_train])
    y_test= np.array([ int(mask_not_blank(ele)) for ele in y_test])
    ...
    print('-'*30)
    model = get_unet()
    model_name = 'unet_seed_1024_epoch_25_aug_rotate_64_80_shiftbn_sgd_srelu_bin.hdf5'
    ...
    if use_pretrain:
        pretrain_model_name = 'unet_seed_1024_epoch_20_aug_rotate_64_80_shiftbn_sgd_srelu_plus10.npy'
        my_names = []
        f = h5py.File('E:\\UltrasoundNerve\\'+pretrain_model_name)
        for layer in model.layers:            
            if layer.name in f.attrs['layer_names']:
                my_names.append(layer.name)       
                # print(layer.name)
        for n in my_names:
            if 'srelu' in n:
                idx = int(n.split('_')[1])
        ...
        my_names.remove('batchnormalization_6')
        my_names.remove('batchnormalization_7')
        # print(my_names)
        model = load_weights(model,'E:\\UltrasoundNerve\\'+pretrain_model_name,my_names,freeze=True)
    # return 
    ...
    np.save('imgs_mask_test_bin.npy', imgs_mask_test)
