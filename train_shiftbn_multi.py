...
def get_unet():
    ...
    output
    '''
    conv10 = Convolution2D(1, 1, 1, activation='sigmoid',name='location_output')(conv9)
    

    location_output = conv10
    bin_output = Flatten()(pool4)
    bin_output = Dense(1024)(bin_output)
    bin_output = LeakyReLU(0.01)(bin_output)
    bin_output = BatchNormalization()(bin_output)
    bin_output = Dense(1024)(bin_output)
    bin_output = LeakyReLU(0.01)(bin_output)
    bin_output = BatchNormalization()(bin_output)
    bin_output = Dense(1,activation='sigmoid',name='bin_output')(bin_output)
    

    output = [location_output,bin_output]
    model = Model(input=inputs, output=output)
    
    # model.compile(optimizer="rmsprop", loss=dice_coef_loss, metrics=[dice_coef],loss_weights=[1., 1.])
    model.compile(optimizer='rmsprop',
              loss={'location_output': dice_coef_loss, 'bin_output': 'binary_crossentropy'},
              loss_weights={'location_output': 1., 'bin_output': 1.}, metrics=[dice_coef])
...
def train_and_predict():
    ...
    imgs_mask_train_bin = np.array([int(sum(x.flatten())>0) for x in imgs_mask_train])
    imgs_mask_valid_bin = np.array([int(sum(x.flatten())>0) for x in imgs_mask_valid])
    
    print('-'*30)
    print('Creating and compiling model...')
    print('-'*30)
    model = get_unet()
    model_name = 'unet_seed_1024_epoch_25_no_aug_64_80_shiftbn_multi.hdf5'
    ...
    if not augmentation:
        model.fit(imgs_train, [imgs_mask_train,imgs_mask_train_bin], batch_size=batch_size, nb_epoch=nb_epoch, verbose=1, shuffle=True,
                  callbacks=[model_checkpoint],
                  validation_data=(imgs_valid,[imgs_mask_valid,imgs_mask_valid_bin])
                  )
        pass
    else:
